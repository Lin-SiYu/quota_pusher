...
from lib.log import logger_info, logger_error
...
class MqBase:
    def __init__(self):
        ...
        self._vhost = settings.get('MQ_VHOST', '/')
    ...
    async def connect(self):
        ...
        try:
            transport, protocol = await aioamqp.connect(host=self._host, port=self._port, login=self._username,
                                                        password=self._pwd, virtualhost=self._vhost)
    ...
    async def producer(self, exchange_name='', exchange_type='', *args, **kwargs):
        ...
        :return:
        '''
        await self._channel.exchange_declare(exchange_name=exchange_name, type_name=exchange_type, durable=True,
                                             *args, **kwargs)
